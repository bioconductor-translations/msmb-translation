...
significance and hypothesis testing in the 1930s by Fisher
([1935](#ref-fisher1935design)) and Neyman and Pearson
([1936](#ref-neyman1936sufficient)), it is well amenable to mathematical
...
Figure @ref(fig:FisherParadigm). Tukey
([1976](#ref-tukey1976exploratory)) emphasized two separate approaches.
The first he termed **exploratory data analysis** (**EDA**). EDA uses
the data themselves to decide how to conduct the statistical analysis.
EDA is built on simple tools for plotting data. EDA is complemented by
**confirmatory data analyses** (CDA): robust inferential methods that do
not rely on complex assumptions to reach scientific conclusions. Tukey
recommended an iterative approach schematized in Figure
@ref(fig:iterativeparadigm) that enable us to see the data at different
resolutions and from different perspectives. This enables the refinement
of our understanding of the data.
...
immediately run into problems [1] , since the number of model parameters
would have to be orders of magnitudes larger than the number of
replicate measurements *n*. At least, this is the case for common
...
regularization techniques Hastie et al.
([2009](#ref-hastie2009elements)), i.e., by requiring that many of the
potential parameters are either zero or at least close to it.
...
important book long treatments (Efron ([2012](#ref-efron2012large))) of
the subject of large scale inference so essential in modern estimation
...
introduce in Chapter @ref(Chap-Generative). We will use examples from
...
data in Chapter @ref(Chap-Graphics). We’ll use the grammar of graphics
and [**ggplot2**](https://cran.r-project.org/web/packages/ggplot2/).

Real biological data often have more complex distributional properties
than what we could cover in Chapter @ref(Chap-Graphics). We’ll use
mixtures that we explore in Chapter @ref(Chap-Mixtures); these enable us
to build realistic models for heterogeneous biological data and provide
solid foundations for choosing appropriate variance stabilizing
transformations.

The large, matrix-like (*n* × *p*) datasets in biology naturally lend
...
@ref(Chap-Clustering). Since clustering only relies on distances, we can
even apply it to data that are not matrix-shaped, as long as there are
objects and distances defined between them.
...
analysis*– in Chapter @ref(Chap-Multivariate). We turn to more
heterogeneous data that combine multiple data types in Chapter
@ref(Chap-MultivaHetero). There, we’ll see nonlinear unsupervised
...
@ref(Chap-Multivariate) to combinations of categorical variables and
multiple assays recorded on the same observational units.

The basic hypothesis testing workflow outlined in Figure
@ref(fig:FisherParadigm) is explained in Chapter @ref(Chap-Testing). We
use the opportunity to apply it to one of the most common queries to
*n* × *p*-datasets: which of the genes (features) are *associated with*
a certain property of the samples, say, disease type or outcome?
However, conventional significance thresholds would lead to lots of
spurious associations: with a false positive rate of *α* = 0.05 we
expect *p**α* = 1000 false positives if none of the *p* = 20000 features
...
@ref(Chap-CountData). Since we’ll draw our example data from an RNA-Seq
experiment, this gives us also an opportunity to discuss models for such
count data, and concepts of *robustness*.

Nothing in biology makes sense except in the light of evolution[2], and
evolutionary relationships are usefully encoded in phylogenetic trees.
We’ll explore networks and trees in Chapter @ref(Chap-Graphs).

A rich source of data in biology is images, and in Chapter
@ref(Chap-Images). we reinforce our willingness to do EDA on all sorts
of heterogeneous data types by exploring feature extraction from images
and spatial statistics.

Finally in Chapter @ref(Chap-Supervised), we will look at statistical
...
methods, and then explore classification in high-dimensional settings.

We wrap up in Chapter @ref(Chap-Design), with considerations on good
...
online resources. One of them is by Wickham and Grolemund
([2016](#ref-wickham2016r)), online at
...
-   there is a function invoked that you have not seen before and that
    does something mysterious,

-   there is a complicated R expression that you don’t understand
    (perhaps involving `apply`-functions or data manipulations from the
...
In Chapters @ref(Chap-Generative) and @ref(Chap-Models), we use
[**base**](https://cran.r-project.org/web/packages/base/) R
...
[**ggplot2**](https://cran.r-project.org/web/packages/ggplot2/) way of
making graphics in Chapter @ref(Chap-Graphics). Besides the powerful
...
and `lapply` constructs for elegant
[**dplyr**](https://cran.r-project.org/web/packages/dplyr/) expressions,
but this requires you to get acquainted with some of the novelties in
there including *tibbles*, the `group_by` function and pipes (`%>%`).

#### Why R and Bioconductor?

There are many reasons why we have chosen to present all analyses on the
R (Ihaka and Gentleman ([1996](#ref-ihaka1996r))) and Bioconductor
(Huber et al. ([2015](#ref-huber2015orchestrating))) platforms:
...
**Data input and wrangling.** Bioconductor packages support the reading
...
keeping these up-to-date with the rapid developments on the instrument
market.
...
publication-quality graphics. We've dedicated Chapter
@ref(Chap-Graphics) to this and practice data visualization extensively
throughout the book.

**Easy to use interactive development environment.** **RStudio** is easy
...
schematized in Figure @ref(fig:iterativeparadigm).
...
[**knitr**](https://cran.r-project.org/web/packages/knitr/) package to
...
**Collaborative environment.** R markdown enables the creation of
websites containing code, text, figures and tables with a minimum of
work.

**Rich data structures.** The Bioconductor project has defined
specialized data containers to represent complex biological datasets.
These help to keep your data consistent, safe and easy to use.

**Interoperability and distributed development.** Bioconductor in
...
**High-level language.** R is an interpreted high-level language. Its
...
languages[3]. R is not a fixed language – throughout its history, it has
...
**Please do not despair if code in the printed version of the book is
...
or packages.** This is fully to be expected and no reason for worries,
...
Efron, Bradley. 2012. *Large-Scale Inference: Empirical Bayes Methods
for Estimation, Testing, and Prediction*. Vol. 1. Cambridge University
Press.
...
Uniformly Most Powerful Tests of Statistical Hypotheses.” *Statistical
Research Memoirs* 1: 113–37.

Tukey, John W. 1976. “Exploratory Data Analysis. 1977.” *Massachusetts:
Addison-Wesley*.
...
[3] Examples include R’s formula interface, the grammar of graphics in
[**ggplot2**](https://cran.r-project.org/web/packages/ggplot2/), the
data manipulation functionality of
[**dplyr**](https://cran.r-project.org/web/packages/dplyr/) and R
markdown.
